...
//	qDebug() << "add tower";
    ...
    squares.clear();
    auto* towerobj = new Tower(pozX, pozY);
    ...
    if(start_game->player->getResources()-towerobj->getCost()>=0 && place) {
        ...
        for (int i = 0;i < max;i++)
        {
            bool connect_enemy_tower = QObject::connect(enemies[i], SIGNAL(sendLocationToTower(int, int)), towerobj, SLOT(checkForEnemy(int, int)));
            bool connect_enemy_bullet = QObject::connect(towerobj->bullet, &Bullet::enemyDamaged, enemies[i], &Enemy::enemyDamaged);
        }
        bool connect_tower_bullet = QObject::connect(towerobj, SIGNAL(sendBulletTarget(int, int)), towerobj->bullet, SLOT(getTargetLocation(int, int)));
    	
        towers.append(towerobj);
...
void MainWindow::SetData(int level)
{
	...
	string levelname="images/level"+to_string(level)+".png";
...
//					qDebug() << "schimbam turn dupa click";
					...
					gameScene->addItem(item);
					towers.remove(i);
                    Tower* towerobj = new Tower((x + rangeX) / 70, (y + rangeY) / 70);
					
                    bool trimis1 = QObject::connect(towerobj, SIGNAL(sendBulletTarget(int, int)), towerobj->bullet, SLOT(getTargetLocation(int, int)));
                    towerobj->checkForEnemy(x / 70 , y/70);
                    gameScene->addItem(towerobj->bullet);
					towers.append(towerobj);
				}
			}
		}
